# Мы устроились на новую работу.
# Бывший сотрудник начал разрабатывать модуль для работы с почтой,
# но не успел доделать его. Код рабочий.
# Нужно только провести рефакторинг кода.
# Создать класс для работы с почтой;
# Создать методы для отправки и получения писем;
# Убрать "захардкоженный" код. Все значения должны определяться как
# аттрибуты класса, либо аргументы методов;
...
class MyEmail:
    ...
    def receive_message(self, folder, header):
        ...
        self.criterion = '(HEADER Subject "%s")' % self.header \
...
def main():
    example_message = MyEmail('smtp.gmail.com', 'imap.gmail.com', \
                              'login@gmail.com', 'qwerty')
    # example_message.send_message('Subject', ['vasya@email.com', \
    # 'petya@email.com'], 'Message', None)
